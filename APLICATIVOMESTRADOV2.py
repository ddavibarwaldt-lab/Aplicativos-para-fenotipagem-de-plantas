...
class ImageDistanceApp:
    def __init__(self, root):
        ...
        # Botão para contagem de objetos
        self.btn_count_objects = tk.Button(self.btn_frame, text="Contagem de Objetos", command=self.toggle_object_count)
        self.btn_count_objects.grid(row=2, column=0)
        ...
        self.counting_objects = False
        self.object_points = []
        self.object_count_box = None
    ...
    def toggle_object_count(self):
        if not self.counting_objects:
            # Ativar contagem
            self.counting_objects = True
            self.object_points.clear()
            self.display_image()
            # Cria a caixinha para mostrar contagem
            if not self.object_count_box:
                self.object_count_box = tk.Entry(self.text_frame, width=10)
                self.object_count_box.pack(pady=5)
            self.object_count_box.delete(0, tk.END)
            self.object_count_box.insert(0, "0")
            self.btn_count_objects.config(text="Finalizar Contagem")
        else:
            # Desativar contagem
            self.counting_objects = False
            self.btn_count_objects.config(text="Contagem de Objetos")
            messagebox.showinfo("Contagem Finalizada", f"Total de objetos contados: {len(self.object_points)}")
    ...
    def on_click(self, event):
        ...
        if self.counting_objects:
            ...
            cv2.circle(self.displayed_img, (x, y), 5, (0, 0, 255), -1)  # Ponto vermelho
            self.refresh_canvas()
            if self.object_count_box:
                self.object_count_box.delete(0, tk.END)
                self.object_count_box.insert(0, str(len(self.object_points)))
            return  # Não processa outros modos

        # Código original de cliques
        if self.btn_set_scale['state'] == tk.NORMAL:
            if len(self.scale_pts) < 2:
                self.scale_pts.append((x, y))
                cv2.circle(self.displayed_img, (x, y), 5, (0,255,0), -1)
                if len(self.scale_pts) == 2:
                    cv2.line(self.displayed_img, self.scale_pts[0], self.scale_pts[1], (0,255,0), 2)
                    self.ask_scale()
                self.refresh_canvas()
        elif self.measuring_perimeter:
            self.perimeter_pts.append((x, y))
            cv2.circle(self.displayed_img, (x, y), 4, (255, 100, 0), -1)
            ...
            cv2.circle(self.displayed_img, (x, y), 5, (0, 200, 255), -1)
            ...
            if len(self.measure_pts) < 2:
                ...
                cv2.circle(self.displayed_img, (x, y), 5, (255, 0, 0), -1)
                if len(self.measure_pts) == 2:
                    self.calc_distance()
                self.refresh_canvas()
